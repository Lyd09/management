...
export function Header() {
  ...
  const commonChangelogPopover = (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={isMobile ? "ghost" : "ghost"} size={isMobile ? "default" : "icon"} className={isMobile ? "w-full justify-start text-base py-3" : ""} aria-label="Ver atualizações recentes">
          <History className="mr-2 h-5 w-5 text-primary hover:text-primary/80" />
          Atualizações
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-96 sm:w-[450px] max-h-[70vh] p-0" side="bottom" align={isMobile ? "start" : "end"}>
      ...
      <SheetContent side="left" className="w-[280px] p-4 flex flex-col">
        ...
        <nav className="flex flex-col gap-3 flex-grow">
          ...
          <SheetClose asChild>
           ...
           {/* O Popover de Histórico precisa ser acionado. No mobile, o botão dentro do sheet faz isso. */}
           {/* Envolvemos o Popover em um div para layout, se necessário, ou o botão trigger direto */}
           <div onClick={(e) => e.stopPropagation()}> {/* Evita que o sheet feche ao abrir o popover */}
            {commonChangelogPopover}
          </div>
        </nav>
  ...
  return (
    <header className="py-4 px-6 border-b border-border sticky top-0 bg-background z-50">
      <div className="container mx-auto flex justify-between items-center">
        ...
        {isMobile === undefined ? ( // Renderiza um placeholder ou nada durante a determinação do isMobile
